import type { Metadata } from 'next'

export const metadata: Metadata = {
    title: 'About Us - USA Licensed Visa Consultant in Sri Lanka',
    description: 'Meet our USA Licensed Consultant Shamlee Bandara with 8+ years experience. Best US visa consultant in Sri Lanka specializing in F-1 student visa, B1/B2 tourist visa, and Schengen visa applications.',
    keywords: [
        'USA licensed consultant Sri Lanka',
        'USA licensed visa consultant',
        'US visa consultant Sri Lanka',
        'best visa consultant Colombo',
        'immigration consultant Sri Lanka',
        'Shamlee Bandara visa consultant',
        'US immigration paralegal',
        'F-1 visa expert Sri Lanka'
    ],
    openGraph: {
        title: 'About Us - USA Licensed Visa Consultant | Global Consulting & Visa Hub',
        description: 'Meet our USA Licensed Consultant with 8+ years of experience helping Sri Lankans achieve their American dream.',
        type: 'website',
    },
}

export default function AboutLayout({
    children,
}: {
    children: React.ReactNode
}) {
    return children
}
